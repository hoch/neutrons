...
export const diffXY = (x, y, n) => {
    ...
    const len = Math.min(keysX.length - 1, keysY.length - 1);
    for (let k = 0; k < len; k++) {
 ...
 * @returns Array
 */
export const integral = (values, n) => {
    // make y enumerated and define x = 1, 2, 3, ...
    let x, y;
    y = enumerate(values);
    x = Object.keys(y).map(Math.floor);
 ...
 */
export const extrema = (values, eps) => {
    // make y enumerated and define x = 1, 2, 3, ...
    let x, y;
    y = enumerate(values);
    x = Object.keys(y).map(Math.floor);
    // call extremaXY version
    const res = extremaXY(x, y, eps);
    res.minlist = res.minlist.map(function (val) {
        const index = Math.floor((val[1] + val[0]) / 2);
 ...
 * @returns {minlist: Array, maxlist: Array}
 */
export const extremaXY = (x, y, eps) => {
    // declare local vars
    let n, s, m, M, maxlist, minlist, i, j;
    // define x & y enumerated arrays
    let enumerate = function (obj) {
        const arr = [];
